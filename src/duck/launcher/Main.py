...
class Launcher(QtGui.QMainWindow):
	def __init__(self):
		...
		#Values
		self.alpha=int(self.conf["alpha"])
		#self.ICON_SIZE=int(conf['icon-size'])
		#self.apps_per_row = math.trunc(((self.s_width/3)-30)/ (int(self.conf["icon-size"])+5))
		#self.apps_per_col = math.trunc(((self.s_height)-60)/ (int(self.conf["icon-size"])+5))
		...
		#Apps widget
		self.appsView= Widgets.AppList(self)
		self.appsView.setApps(Apps.info(""))
		self.appsView.setAppIconSize(self.conf["icon-size"])
		self.appsView.setFont(self.conf["font"])
		self.appsView.setFontColor(255,255,255)
		self.appsView.setBackgroundColor(self.conf["r"],self.conf["g"],self.conf["b"])
		self.appsView.setGeometry(2,60,self.s_width/3-20,self.s_height-60)
		self.appsView.hide()
	...
	def paintEvent(self,e):
		...
		qp.setBrush(QtGui.QColor(int(self.conf['r2']),int(self.conf['g2']),int(self.conf['b2']),self.alpha))
		...
		if self.current_state == "half_open":
			...
			icon = QtGui.QIcon("{}apps.svg".format(ICON_DIR))
			icon.paint(qp,5,ICO_TOP*0+5,ICO_TOP,ICO_TOP-6)
			#Files
			icon = QtGui.QIcon("{}file.svg".format(ICON_DIR))
			##temp_file
			icon.paint(qp,5,ICO_TOP*1+5,ICO_TOP,ICO_TOP-6)
			#Settings
			icon = QtGui.QIcon("{}settings.svg".format(ICON_DIR))
			icon.paint(qp,5,ICO_TOP*2+5,ICO_TOP,ICO_TOP-6)
			#Star
			icon = QtGui.QIcon("{}star.svg".format(ICON_DIR))
			...
			for i,a in enumerate(self.dock_apps):
				try:
					if a["name"] in [e["name"] for e in self.ow_in_dock]:
						qp.setBrush(QtGui.QColor(self.fg_color[0],self.fg_color[1],self.fg_color[2],50))
						qp.drawRect(0,self.OPEN_STATE_TOP+ICO_TOP*i+8,self.pos_x+7,ICO_TOP-1)
					QtGui.QIcon(Apps.ico_from_name(a['icon'])).paint(qp,0,self.OPEN_STATE_TOP+ICO_TOP*i+10,self.pos_x+7,ICO_TOP-5)
				except KeyError:
			...
			if len(self.open_windows)>0:
				...
				if len(ow_list)>0:
					qp.drawRect(4, after_dapps-15, self.pos_x-1,1)
		...
		if self.current_state=="open":
			close=QtGui.QIcon("{}remove.svg".format(ICON_DIR))
			...
			if self.activity=="star" :
				...
				all_rows=0
				try:
					STAR_ICO=int(self.conf["icon-size"])*1.8
					blocks_l=pickle.loads(self.conf["blocks"])
					for i,b in enumerate(blocks_l):
						all_stuff = Config.get_from_block(b)
						apps_per_row = math.trunc(((self.s_width/3)-30)/STAR_ICO)
						...
						for j, thing in enumerate(all_stuff):
							...
							x_pos = STAR_ICO*(j-(row_pos*apps_per_row))+40
							y_pos = (row_pos*STAR_ICO+20)+h+30
							if thing['type']=='app':
								val=str(thing["value"])
								ico = Apps.ico_from_app(val)
								...
								pass
							if icon!=None:
								icon.paint(qp, x_pos+15,y_pos+15, STAR_ICO-50,STAR_ICO-50)
								rect = QtCore.QRectF(x_pos-10, y_pos+STAR_ICO-30, STAR_ICO, 30)
		...
		if self.current_state!="half_open":
			qp.setPen(QtCore.Qt.NoPen)
			qp.setBrush(QtGui.QColor(self.fg_color[0],self.fg_color[1],self.fg_color[2],10))
			qp.drawRect(self.pos_x-14,0,21,self.s_height)
			if self.plugin==False:
				qp.setBrush(QtGui.QColor(self.fg_color[0],self.fg_color[1],self.fg_color[2],255))
			else:
				qp.setBrush(QtGui.QColor(self.font_color[0],self.font_color[1],self.font_color[2],255))
	...
	def mousePressEvent(self,e):
		...
		if self.current_state=="half_open":
			if 0<x_m<self.HALF_OPEN_POS:
				if y_m<self.ICO_TOP:
					self.appRect=QtCore.QRectF(0,2, self.pos_x+7,self.ICO_TOP)
					self.drawAppRect=True
				if self.ICO_TOP<y_m<self.ICO_TOP*2:
					self.appRect=QtCore.QRectF(0,self.ICO_TOP+2,self.pos_x+7,self.ICO_TOP)
					self.drawAppRect=True
				if self.ICO_TOP*2<y_m<self.ICO_TOP*3:
					self.appRect=QtCore.QRectF(0,self.ICO_TOP*2+2,self.pos_x+7,self.ICO_TOP)
					self.drawAppRect=True
				if self.ICO_TOP*3<y_m<self.ICO_TOP*4:
					self.appRect=QtCore.QRectF(0,self.ICO_TOP*3+2,self.pos_x+7,self.ICO_TOP)
					self.drawAppRect=True
			for i,a in enumerate(self.dock_apps):
				if self.OPEN_STATE_TOP+self.ICO_TOP*i+10<y_m<self.OPEN_STATE_TOP+self.ICO_TOP*(i+1)+10:
					self.appRect=QtCore.QRectF(0, self.OPEN_STATE_TOP+self.ICO_TOP*i+7, self.pos_x+7,self.ICO_TOP)
					self.drawAppRect=True
			if len(self.open_windows)>0:
				...
				for i,w in enumerate(ow_list):
					...
					if y_pos<y_m<y_pos+self.ICO_TOP:
						self.appRect=QtCore.QRectF(0, y_pos,self.pos_x+7,self.ICO_TOP)
						self.drawAppRect=True
			...
			for i, app in enumerate(self.allApps):
				...
				if app_page==self.app_page_state:
					...
					if x_pos<x_m<x_pos+int(self.conf["icon-size"]) and y_pos<y_m<y_pos+int(self.conf["icon-size"]):
						...
						self.drawAppRect=True
		'''
		if self.current_state=="open" and self.activity=="star":
			STAR_ICO=int(self.conf["icon-size"])*1.8
			...
			for i,b in enumerate(blocks):
				...
				for j, thing in enumerate(all_stuff):
					...
					x_pos = STAR_ICO*(j-(row_pos*apps_per_row))+40
					y_pos = (row_pos*STAR_ICO+20)+h+30
					if x_pos-10<x_m<x_pos-10+STAR_ICO and y_pos<y_m<y_pos+STAR_ICO and x_m<self.pos_x-self.SIZE-3:
						self.appRect=QtCore.QRectF(x_pos-10,y_pos,STAR_ICO,STAR_ICO)
						self.drawAppRect=True
...
'''
			...
			if 0<x_m<self.HALF_OPEN_POS:
				if e.button()==QtCore.Qt.LeftButton:
					if y_m<self.ICO_TOP:
						...
						self.current_text=''
						self.open_it()
						apps = Apps.info(self.current_text)
						self.appsView.setApps(apps)
						self.appsView.setAppIconSize(60)
						self.appsView.show()
						#self.allApps=Apps.info(self.current_text)
					if self.ICO_TOP<y_m<self.ICO_TOP*2:
						...
						if html!=None:
							self.open_it()
							self.plugin=True
							self.pl_logo="{}file.svg".format(ICON_DIR)
					...
					if  self.s_height-self.HALF_OPEN_POS-5<y_m:
						if self.sys_win.isHidden():
							#self.open_win.close()
							self.sys_win.update_all(self.conf)
							self.sys_win.show()
	...
	def open_it(self):
		...
		while self.pos_x<self.s_width/3-4.5:
			...
			if self.pos_x>self.s_width/3-4.5:
				self.pos_x=self.s_width/3-4.5
			self.repaint()
			QtGui.QApplication.processEvents()
		...
		QtGui.QApplication.processEvents()
	def close_it(self):
		if self.appsView.isHidden()==False:
			self.appsView.hide()
		if self.plugin==True:
			self.webview.hide()
			self.close_pl_rect()
	...
	def open_pl_rect(self):
		self.pl_rect_pos=0
		while self.pl_rect_pos<(self.pos_x+7):
			self.pl_rect_pos+=int(self.conf["animation-speed"])*2
			if self.pl_rect_pos>self.pos_x+7:
				self.pl_rect_pos=self.pos_x+7
			self.repaint()
			QtGui.QApplication.processEvents()
		self.pl_rect_pos=self.pos_x+7
		self.update()
	...
	def keyPressEvent(self, e):
		...
		if e.key()==QtCore.Qt.Key_Backspace:
			...
			if self.parent.plugin==False:
				...
				self.parent.appsView.setApps(self.parent.allApps)
			self.parent.update()
			...
			if pl.isAlright():
				...
				#self.parent.setAttribute(QtCore.Qt.WA_X11NetWmWindowTypeDock,False)
				#self.parent.setGeometry(0,self.parent.top_pos,self.parent.pos_x+12,self.parent.s_height)
...
class DBusWidget(dbus.service.Object):
	...
	def updateOtherWindows(self):
		if self.parent.dock_options_win.isHidden()==False:
			self.parent.dock_options_win.update_all(self.conf)
		if self.parent.sys_win.isHidden()==False:
			self.parent.sys_win.update_all(self.conf)
	...
	def setR1(self,v):
		...
		self.parent.appsView.updateStyle()
	@dbus.service.method("org.duck.Launcher", in_signature='', out_signature='')
	def setG1(self,v):
		self.conf["g"]=v
		self.parent.conf=self.conf
	...
	def setIconSize(self,v):
		...
		self.parent.conf=self.conf
		if self.parent.ICON_SIZE!=int(self.conf['icon-size']):
			self.parent.ICON_SIZE=int(self.conf['icon-size'])
			self.parent.appsView.setAppIconSize(int(self.conf['icon-size']))
			self.parent.update()
	@dbus.service.method("org.duck.Launcher", in_signature='', out_signature='')
	def setLauncherWidth(self,v):
		...
		self.parent.setGeometry(0,self.parent.top_pos,self.parent.pos_x+10,self.parent.s_height)
	...
	def setFont(self,v):
		...
		self.parent.appsView.setFont(v)
		self.parent.appsView.updateStyle()
	@dbus.service.method("org.duck.Launcher", in_signature='',out_signature="")
	def setFontR(self,v):
		self.conf["font-r"]=v
