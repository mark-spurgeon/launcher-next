	...
	for w in win:
			if  'NORMAL' in str(w.get_window_type()):
				if "ducklauncher!!!"==w.get_name():
					pass		
				elif w.is_sticky()!=True and "ducklauncher!!"!=w.get_name() and "duckpanel!!"!=w.get_name():
					window={}	
					window['id']=w.get_xid()
					...
					if ico==None:
						ico = Apps.ico_from_app(window["app"])
					if ico==None:
						pix=w.get_icon()
						...
						try:
    							...
    							os.mkdir(home)
						#print window
						img_name=str(window["title"]).encode("ascii","ignore").replace(" ","").replace(".","").replace("/","").replace("\"","").lower()
        ...
        def run(self):
        	while 1 : 
        		try:
        			...
        			print "[Duck Launcher] Error:" , traceback.print_exc()
