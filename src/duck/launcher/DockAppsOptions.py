...
class Window(QtGui.QMainWindow):
	...
	def paintEvent(self,e):
		...
		qp.drawRoundedRect(QtCore.QRect(10,0,self.width-9,self.height+2),3,3)
		qp.setBrush(QtGui.QColor(0,0,0,30))
		qp.drawRoundedRect(QtCore.QRect(8,0,self.width-7,self.height+3),4,4)
		#background
		qp.setBrush(QtGui.QColor(int(self.r),int(self.g),int(self.b)))
		qp.drawRect(QtCore.QRectF(10,0,self.width-10,self.height))
		qp.setPen(QtCore.Qt.NoPen)
		qp.setBrush(QtGui.QColor(255,255,255,255))
		qp.drawRect(9,0,5,self.height)
		icon=QtGui.QIcon("{}win.svg".format(ICON_DIR))
		...
		qp.setPen(QtGui.QColor(0,0,0,60))
		...
		if self.state=="normal":
			...
			#open
			o_s_rect=QtCore.QRectF(56,31,self.width-10,30)
			o_rect=QtCore.QRectF(55,30,self.width-10,30)
			qp.setPen(QtGui.QColor(0,0,0,60))
			qp.drawText(o_s_rect,QtCore.Qt.AlignVCenter,"Open")
			qp.setPen(QtGui.QColor(255,255,255,255))
			qp.drawText(o_rect,QtCore.Qt.AlignVCenter,"Open")
			removeIcon=QtGui.QIcon("{}open.svg".format(ICON_DIR))
			removeIcon.paint(qp, 25,35,20,20)
			#move
			m_s_rect=QtCore.QRectF(56,61,self.width-10,30)
			m_rect=QtCore.QRectF(55,60,self.width-10,30)
			qp.setPen(QtGui.QColor(0,0,0,60))
			qp.drawText(m_s_rect, QtCore.Qt.AlignVCenter,"Move")
			qp.setPen(QtGui.QColor(255,255,255))
			qp.drawText(m_rect, QtCore.Qt.AlignVCenter,"Move")
			removeIcon=QtGui.QIcon("{}move.svg".format(ICON_DIR))
			removeIcon.paint(qp, 25,65,20,20)

			#remove
			r_s_rect=QtCore.QRectF(56,91,self.width-10,30)
			r_rect=QtCore.QRectF(55,90,self.width-10,30)
			qp.setPen(QtGui.QColor(0,0,0,60))
			qp.drawText(r_s_rect, QtCore.Qt.AlignVCenter,"Remove")
			qp.setPen(QtGui.QColor(255,255,255))
			qp.drawText(r_rect, QtCore.Qt.AlignVCenter,"Remove")
			removeIcon=QtGui.QIcon("{}remove.svg".format(ICON_DIR))
			removeIcon.paint(qp, 25,95,20,20)
			...
			upicon=QtGui.QIcon("{}up.svg".format(ICON_DIR))
			upicon.paint(qp, self.width/2+35,35,40,40)
			downicon=QtGui.QIcon("{}down.svg".format(ICON_DIR))
	...
	def mouseReleaseEvent(self,e):
		x_m, y_m = e.x(),e.y()
		self.drawButtonRect=False
		if self.state=="normal" and e.button()==QtCore.Qt.LeftButton:
			if 15<x_m<self.width-15 and 30<y_m<55:
			...
			if 15<x_m<self.width-15 and 87<y_m<115:
				...
				self.close()
		elif self.state=="moving" and e.button()==QtCore.Qt.LeftButton:
			if 35<y_m<80 and self.width/2+35<x_m<self.width/2+75:
				print "Moving Ùp"
