...
class get():
	def __init__(self,query):
		self.plugin_path= "{}plugins".format(datadir.getDir()) #os.path.join(os.path.expanduser("~"),".duck-plugins")
		
		plugins=[]
		for p in os.listdir(self.plugin_path):
			if os.path.isdir(os.path.join(self.plugin_path,p)):
				if os.path.isfile(os.path.join(self.plugin_path,p, "plugin.py")):			
					sys.path.insert(0, os.path.join(self.plugin_path,p))
					plugin =imp.load_source(str(p),os.path.join(self.plugin_path,p,"plugin.py"))
	...
	def html(self,color=None,font=None):
		...
		return html
	def info(self):
		path = os.path.join(self.plugin_path,self.plugin_name,"manifest.json")
	...
	def getLogo(self):
		path= os.path.join(self.plugin_path, self.plugin_name,"logo.svg")
		if os.path.isfile(path):
			return path
		else:
			return None
def get_plugin_names():
	plugin_path= "{}plugins".format(datadir.getDir())
	plugins=[]
...
class Repo():
	...
	##The functions marked with a '_' directly find information from github. 
	...
	def _getPluginInfo(self,plugin):
		try:
	...
	def _getPluginLogoUrl(self,plugin):
		try:
	...
	def _getPluginsFromGithub(self):
		ps=self._getPlugins()
		if ps!=None:
			all_plugins=[]
			for p in ps:
				d = self._getPluginInfo(p["path"])
				t = self._getPluginLogoUrl(p["path"])
	...
	#Finds whether a plugin is installed or not
	def getStatus(self,pluginDict):
		f="{0}plugins/{1}/manifest.json".format(datadir.getDir(),pluginDict["path"])
		if os.path.isfile(f):
			s = open(f,"r")
			d = json.loads(s.read())
			s.close()
			...
			return "uninstalled"
	#Finds all apps, from Duck Launcher's website, like that it's quicker for the Plugin Store to find 
	def getAllApps(self):
		r = urllib2.urlopen("http://the-duck.github.io/plugins/all.json")
		str_r=r.read()
		json_r=json.loads(str_r)
		new_app_list=[]
		for d in json_r:
			new_d =d
			new_d["status"]=self.getStatus(d)
			new_app_list.append(new_d)
		return new_app_list
	def installPlugin(self,plugin):
		destination=os.path.join("{}plugins/".format(datadir.getDir()),plugin)
...
if __name__=="__main__":
	r = Repo()
	p = r._pluginStoreGetApps()
	l = json.dumps(p,indent=4)